...
const MLX_SOURCE = process.env.MLX_SOURCE || '/tmp/mlx-source/python/src';
...
async function main() {
  ...
  let parsed;
  try {
    parsed = JSON.parse(readFileSync('generated/bindings.json', 'utf-8'));
  } catch (e) {
    console.error('ERROR: generated/bindings.json not found. Run: npm run generate');
    process.exit(1);
  }
